...
export function Footer() {
  return (
    <footer className="border-t border-[#E9DECB] bg-[#F2E6D3] px-4 py-12 sm:px-6">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-lg font-semibold text-[#2A2420]">Live Flow</p>
          <p className="mt-1 text-sm text-[#5C534C]">Voice, in flow.</p>
          <a
            href={DOWNLOAD_URL}
        ...
        <nav className="flex flex-wrap gap-x-5 gap-y-2" aria-label="Footer">
          {links.map((link) => (
            <a
              ...
              className="text-sm text-[#5C534C] transition hover:text-[#2A2420]"
            >
              {link.label}
            </a>
          ))}
      ...
      <p className="mx-auto mt-10 max-w-6xl text-xs text-[#5C534C]">Made for macOS</p>
    </footer>
  );
}
