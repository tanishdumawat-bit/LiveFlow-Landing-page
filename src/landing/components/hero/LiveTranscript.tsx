...
export function LiveTranscript({ stage, accent }: Props) {
  ...
  if (stage === 'idle' || stage === 'listening') {
    return (
      <div className="mx-auto flex h-11 max-w-lg items-center justify-center" aria-hidden="true">
        <motion.span
          className="text-xs tracking-wide text-muted/70"
  ...
  return (
    <div
      className="mx-auto w-full max-w-lg overflow-hidden rounded-2xl border border-border bg-white/90 px-4 py-2.5 shadow-card backdrop-blur-sm"
      role="status"
      aria-live="polite"
    >
      <AnimatePresence mode="wait">
        {showRaw && !showClean && (
          <motion.p
            ...
            className="flex flex-wrap gap-x-1.5 gap-y-1 text-sm text-ink"
          >
            {HERO_RAW_WORDS.map((word, i) => {
              ...
              return (
                <motion.span
                  ...
                  style={{ color: isFiller && processing ? theme.muted : undefined }}
                >
                  {word}
                </motion.span>
              );
